...
describe('RefreshButton Component', () => {
  ...
  describe('Accessibilité', () => {
    ...
    it('devrait avoir un contraste suffisant', () => {
      ...
      expect(button).toHaveClass('border-chat-border-soft');
    });
  });
  ...
  describe('Classes CSS personnalisées', () => {
    it('devrait accepter les classes CSS personnalisées', () => {
      ...
      // "Rafraîchir" est dans le div interne (select + bouton) ; la classe
      // personnalisée est appliquée au conteneur externe (un niveau au-dessus).
      const container = screen.getByText('Rafraîchir').closest('div')?.parentElement;
...
describe('RefreshButton Integration', () => {
  it('devrait fonctionner avec différents callbacks', async () => {
    const customCallback = vi.fn(() => Promise.resolve());
