...
describe('PATCH /api/conversations/[id]/rename', () => {
  ...
  it('AC #5 - devrait retourner 401 si utilisateur non authentifié', async () => {
    const request = {
      headers: { get: vi.fn() },
  ...
  it('AC #5 - devrait retourner 400 si titre manquant', async () => {
    const request = {
  ...
  it('AC #5 - devrait retourner 400 si titre vide', async () => {
    const request = {
      headers: { get: vi.fn((key: string) => key === 'x-user-id' ? 'user-123' : null) },
      nextUrl: { pathname: '/api/conversations/conv-123/rename' },
    ...
    expect(response.status).toBe(400)
    const data = await response.json()
    expect(data.error).toContain('Le titre ne peut pas etre vide')
  })

  it('AC #5 - devrait retourner 400 si titre trop long', async () => {
    ...
    expect(response.status).toBe(400)
    const data = await response.json()
    expect(data.error).toContain('Le titre ne peut pas depasser 200 caracteres')
  })

  it('AC #5 - devrait retourner 404 si conversation non trouvée', async () => {
  ...
  it('AC #5 - devrait retourner 200 et renommer la conversation', async () => {
    // Mock: conversation existe et renommage réussi
