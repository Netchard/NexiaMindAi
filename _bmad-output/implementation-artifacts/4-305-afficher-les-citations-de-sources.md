...
- [ ] **Task 0 — Prérequis : Vérifier le contrat API et le format des citations**
  - [ ] Vérifier que `/api/chat/message` retourne bien un tableau `sources` avec les métadonnées nécessaires (path, type, relevance)
  - [ ] Confirmer que le format des citations dans `content` est `[Source: /chemin/vers/document]` comme spécifié dans architecture.md
  - [ ] Valider que le formatter backend (`lib/rag/formatter.js`) génère bien la section "Sources :" avec les liens
  - [ ] Vérifier que `getSourceUrl()` existe et génère des URLs valides pour chaque type de source (supabase, gitlab, nexia)

- [ ] **Task 1 — Type SourceCitation** (AC: #1, #2, #3, #4, #6)
  - [ ] Créer `src/types/citations.ts` : types TypeScript pour les citations (SourceCitation, SourceType, etc.)
  - [ ] Définir l'interface `SourceCitation` : { id: string, path: string, type: SourceType, relevance?: number, url?: string }
  - [ ] Définir le type `SourceType` : 'supabase' | 'gitlab' | 'nexia' | 'upload'
  - [ ] Définir l'interface pour la section Citations dans ChatMessageData
  - [ ] Exporter tous les types depuis l'index du dossier types

- [ ] **Task 2 — Composant SourceCitation** (AC: #1, #2, #3, #4, #6, #7, #8)
  - [ ] Créer `src/components/SourceCitation/index.tsx` (barrel export)
  - [ ] Créer `src/components/SourceCitation/SourceCitation.tsx` : composant pour une seule citation
  - [ ] Créer `src/components/SourceCitation/SourceCitationList.tsx` : conteneur pour la liste des citations
  - [ ] Créer `src/components/SourceCitation/types.ts` : types spécifiques aux composants
  - [ ] Styliser selon le design system (couleurs corail/encre, typographie, espacements)
  - [ ] Accessibilité : chaque citation a un `aria-label` descriptif, gestion du focus
  - [ ] Responsive : afficher en ligne ou en colonne selon l'espace disponible
  - [ ] Icône de lien externe (→) pour indiquer l'ouverture dans nouvel onglet

- [ ] **Task 3 — Client API pour les URLs de source** (AC: #2, #4)
  - [ ] Créer `src/lib/api/sources.ts` : fonctions utilitaires pour générer les URLs
  - [ ] Implémenter `getSourceUrl(source: SourceCitation): string` pour chaque type de source
  - [ ] Gérer les URLs Supabase : construction de l'URL de visualisation dans l'interface Supabase
  - [ ] Gérer les URLs GitLab : construction du lien vers le fichier dans le projet GitLab
  - [ ] Gérer les URLs Nexia : construction du lien vers le document dans Nexia GED
  - [ ] Gestion d'erreur : retourner une URL par défaut ou null si le type est inconnu

- [ ] **Task 4 — Intégration avec les messages** (AC: #1, #5, #6, #10)
  - [ ] Étendre `ChatMessageData` pour inclure un champ optionnel `citations: SourceCitation[]`
  - [ ] Modifier `src/components/Chat/ChatMessage.tsx` pour afficher la section Citations sous le contenu du message assistant
  - [ ] Passer les citations au composant ChatMessage depuis ChatMessageList
  - [ ] Ne pas afficher la section Citations pour les messages utilisateur (seulement assistant)
  - [ ] Conserver le contenu existant intact (compatibilité avec ST-303)

- [ ] **Task 5 — Gestion d'état et parsing** (AC: #1, #2, #3, #5)
  - [ ] Parser le champ `sources` de la réponse API pour créer des objets SourceCitation
  - [ ] Générer les URLs pour chaque citation en utilisant `getSourceUrl()`
  - [ ] Numéroter les citations séquentiellement (1, 2, 3, ...)
  - [ ] Gérer les cas où `sources` est undefined ou vide (pas de section affichée)
  - [ ] Gérer les erreurs de parsing ou de génération d'URL

- [ ] **Task 6 — Expérience utilisateur** (AC: #1, #2, #3, #4, #7)
  - [ ] Afficher la section Citations avec un titre clair ("Sources :" ou "📚 Sources")
  - [ ] Style visuel distinct pour séparer les citations du contenu du message
  - [ ] Espacement adéquat entre le contenu et les citations
  - [ ] Surligner ou indiquer visuellement les citations dans le texte du message
  - [ ] Tooltips ou infobulles pour expliquer comment accéder aux documents

- [ ] **Task 7 — Tests** (tous les AC)
  - [ ] Tests unitaires pour `SourceCitation` : rendu, liens, accessibilité
  - [ ] Tests unitaires pour `SourceCitationList` : intégration, numérotation
  - [ ] Tests du client API : génération des URLs pour chaque type de source
  - [ ] Tests d'intégration : citations affichées correctement dans ChatMessage
  - [ ] Tests du parsing : conversion des sources API vers SourceCitation
  - [ ] Tests E2E : parcours utilisateur complet avec citations (nécessite environnement complet)
...
**Status:** ready-for-dev  
**Completed:** Analyse exhaustive et création du contexte ultime pour le développeur  
**Next:** Implémentation par le dev agent  
**Blockers:** Aucun identifié - le backend génère déjà les citations  
**Dependencies:** Contrat API /api/chat/message déjà implémenté (vérifié dans architecture.md)

### 📊 Résumé de l'implémentation attendue
...
- ⏳ AC #1 - Section "Sources" visible (à implémenter)
- ⏳ AC #2 - Liens cliquables (à implémenter)
- ⏳ AC #3 - Numérotation des sources (à implémenter)
- ⏳ AC #4 - Ouverture dans nouvel onglet (à implémenter)
- ⏳ AC #5 - Affichage des métadonnées (à implémenter)
- ⏳ AC #6 - Style cohérent (à implémenter)
- ⏳ AC #7 - Accessibilité WCAG 2.1 AA (à implémenter)
- ⏳ AC #8 - Responsive design (à implémenter)
- ⏳ AC #9 - Gestion d'erreur (à implémenter)
- ⏳ AC #10 - Intégration avec ST-303 (à implémenter)
